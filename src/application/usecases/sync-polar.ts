...
export class SyncPolarUseCase {
  ...
  async execute(userId: string, polarUserId: string, accessToken: string, startDate: Date, endDate: Date): Promise<void> {
    try {
      ...
      const uniqueActivities = [...new Map(mappedActivities.map(a => [`${a.userId}-${a.date.toISOString().split('T')[0]}`, a])).values()];
      ...
      const uniqueSleep = [...new Map(mappedSleep.map(s => [`${s.userId}-${s.date.toISOString().split('T')[0]}`, s])).values()];
